...
class REMOTE_SCAN_MODE(Enum):
    SYNC = "sync"
    ASYNC = "async"


DEFAULT_SCAN_DEFINITION_NAME = "Soda Core CLI"
...
class Scan(*scan_extra_mixins):
    def __init__(self):
        ...
        self._remote_mode: REMOTE_SCAN_MODE = REMOTE_SCAN_MODE.SYNC.value
        self._scan_definition_name: str | None = None
        self.log_version()
        self.scan_id = None
    ...
    def set_remote_mode(self, mode: REMOTE_SCAN_MODE):
        self._remote_mode = mode

    def is_remote(self):
        return self._scan_type == ScanType.Remote
    ...
    def set_remote(self, is_remote=True, remote_mode: bool = REMOTE_SCAN_MODE.SYNC.value):
        self.set_remote_mode(remote_mode)
        if is_remote:
            self.set_scan_type(ScanType.Remote)
        else:
            self.set_scan_type(None)
    ...
    def setup_datasources_checks(self):
    ...
    def get_invalid_attributes_checks(self) -> list[Check]:
    ...
    def scan_summary(self) -> ScanResult:
        ...
        checks_not_evaluated_count = checks_count - checks_pass_count - checks_warn_count - checks_fail_count

        if checks_count == 0:
            self._logs.warning("No valid checks found, 0 checks evaluated.")
        if checks_not_evaluated_count:
            self._logs.info(f"{checks_not_evaluated_count} checks not evaluated.")
        if error_count > 0:
        ...
        return ScanResult(
            ...
            checks_pass_count=checks_pass_count,
            checks_warn_count=checks_warn_count,
            checks_fail_count=checks_fail_count,
            checks_not_evaluated_count=checks_not_evaluated_count,
            error_count=error_count,
        )

    def save_scan_result_to_file(self, file_path: str, scan_metadata: dict):
        logger.info(f"Saving scan results to {file_path}")
    ...
    def execute_remote(self) -> ScanResult | None:
        soda_cloud = self._configuration.soda_cloud
        scan_result = None

        if not self._scan_definition_name or self._scan_definition_name == DEFAULT_SCAN_DEFINITION_NAME:
            raise RemoteScanException("Scan definition name is required for remote scans.", None)

        def handle_interrupt(sig, frame):
            self._logs.info("Scan interrupted. Sending cancellation request to Soda Cloud.")
            response, _ = soda_cloud.delete_remote_scan(self.scan_id)

            if response.ok:
                self._logs.info("Cancellation request sent successfully.")
            else:
                self._logs.error("Cancellation request failed.")
        ...
        try:
            start_headers = self._configuration.soda_cloud.start_remote_scan(
                scan_definition_name=self._scan_definition_name, remote_mode=self._remote_mode
            )
            status_location = start_headers.get("location")
            scan_id = start_headers.get("x-soda-scan-id")

            self.scan_id = scan_id

            self._logs.info(f"Remote Scan started.")
            self._logs.info(f"Scan ID: {scan_id}")
            self._logs.info(f"Status url: {status_location}")

            if self._remote_mode == REMOTE_SCAN_MODE.SYNC.value:
                self._logs.info("Waiting for remote scan to complete.")
                scan_run = soda_cloud.poll_remote_scan_finished(scan_id)

                if scan_run:
                    self._logs.info("Remote scan completed.")
                    self._logs.info("Fetching scan logs.")

                    # TODO implement get logs pager
                    scan_logs = soda_cloud.get_remote_scan_logs(scan_id)
                    self._logs.info("Scan logs fetched.")

                    if not scan_logs:
                        self._logs.warning("No logs found.")
                    logs = soda_cloud.parse_remote_logs(scan_logs)
                    ...
                    raise Exception("Unexpected state of remote scan.")
            else:
                # TODO make sure nothing else happens after this.
                self._logs.info("Remote scan started in async mode.")

        except Exception as e:
            raise RemoteScanException(str(e), e)
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            if self._scan_type == ScanType.Remote:
                self._logs.info(f"Remote scan {self._remote_mode} mode")
                scan_result = self.execute_remote()
            else:
                self.setup_datasources_checks()
                ...
                if not invalid_checks:
                    ...
                    scan_result = self.scan_summary()

            # Evaluate the scan results and set the exit code
            if scan_result:
                if scan_result.error_count > 0:
                    exit_value = EXIT_CODE_RESULTS_ERRORS
                elif scan_result.checks_fail_count > 0:
                    exit_value = EXIT_CODE_RESULTS_FAILURES
                elif scan_result.checks_warn_count > 0:
                    exit_value = EXIT_CODE_RESULTS_WARNINGS
            else:
                # Non-remote scans and remote sync scans should have result.
                if not self.is_remote() or self.is_remote() and self._remote_mode == REMOTE_SCAN_MODE.SYNC.value:
                    exit_value = EXIT_CODE_NO_RESULTS_EXCEPTION
        except RemoteScanException as e:
            exit_value = EXIT_CODE_NO_RESULTS_EXCEPTION
            if self._configuration.soda_cloud and self._configuration.soda_cloud.soda_cloud_trace_ids:
                self._logs.info(f"Soda Cloud trace IDs: '{self._configuration.soda_cloud.soda_cloud_trace_ids}'")
            self._logs.error("Error occurred while executing remote scan.", exception=e)
        except Exception as e:
            exit_value = EXIT_CODE_RESULTS_ERRORS
            ...
            try:
                ...
                # Insert scan results to Soda Cloud, send telemetry - in local scan mode only.
                if self._configuration.soda_cloud and not self.is_remote():
                    if scan_result:
                        # Telemetry data
                        soda_telemetry.set_attributes(
                            {
                                "pass_count": scan_result.checks_pass_count,
                                "error_count": scan_result.error_count,
                                "failures_count": scan_result.checks_fail_count,
                            }
                        )
                        scan_metadata = {
                            "checks": {
                                "count": scan_result.checks_count,
                                "pass": scan_result.checks_pass_count,
                                "warn": scan_result.checks_warn_count,
                                "fail": scan_result.checks_fail_count,
                                "evaluated": scan_result.checks_count - scan_result.checks_not_evaluated_count,
                                "notEvaluated": scan_result.checks_not_evaluated_count,
                            }
                        }
                    ...
                    if response and "scanReference" in response:
                        ...
                        raise Exception("No scanReference, Cloud Scan insertion failed")
        ...
        if self._scan_results_file is not None:
            try:
                self.save_scan_result_to_file(self._scan_results_file, scan_metadata)
            except Exception as e:
                exit_value = EXIT_CODE_RESULTS_ERRORS
    ...
    def get_remote_scan_run(self, scan_id: str | None = None) -> RemoteScanRun:
        if not scan_id:
            scan_id = self.scan_id

        if not scan_id:
            raise Exception("Scan ID is required to get remote scan run. Have you started a remote scan?")
        return self._configuration.soda_cloud.get_remote_scan_run(scan_id)
    ...
    def get_remote_scan_logs(self, scan_id: str | None = None) -> str:
        ...
        if not scan_id:
            raise Exception("Scan ID is required to get remote scan logs. Have you started a remote scan?")

        soda_cloud = self._configuration.soda_cloud

        return soda_cloud.parse_remote_logs(soda_cloud.get_remote_scan_logs(scan_id))

    def get_remote_logs_text(self, scan_id: str | None = None) -> str:
        logs = self.get_remote_scan_logs(scan_id)
        return self.__logs_to_text(logs)
