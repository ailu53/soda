...
class ConfigurationParser(Parser):
    ...
    def parse_soda_cloud_cfg(self, config_dict: dict):
        ...
        token = None
        # Use token only if api_key and api_secret are not provided, ignore it otherwise.
        if "token" in config_dict:
            if not api_key and not api_secret:
                token = config_dict.get("token")
            if api_key and api_secret:
                self.logs.info(
                    "Both Soda Cloud api keypair and token are provided. Ignoring token.",
                )
        return SodaCloud(
            api_key_id=api_key,
    ...
    @staticmethod
    def parse_dbt_cloud_cfg(config_dict: dict):
