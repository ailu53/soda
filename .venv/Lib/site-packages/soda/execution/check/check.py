...
class Check(ABC):
    ...
    def create(
        ...
        elif instance_but_not_subclass(check_cfg, AnomalyDetectionMetricCheckCfg):
        ...
        # Reconciliation checks
            ...
            from soda.sodacl.reconciliation_row_diff_check_cfg import DEEPDIFF, SIMPLE

            if check_cfg.strategy == SIMPLE:
                from soda.execution.check.reconciliation_row_diff_simple_check import (
                    ReconciliationRowDiffSimpleCheck,
                )

                return ReconciliationRowDiffSimpleCheck(check_cfg, data_source_scan, partition)
            if check_cfg.strategy == DEEPDIFF:
                from soda.execution.check.reconciliation_row_diff_deepdiff_check import (
                    ReconciliationRowDiffDeepDiffCheck,
                )

                return ReconciliationRowDiffDeepDiffCheck(check_cfg, data_source_scan, partition)
            raise RuntimeError(f"Unsupported row reconciliation strategy: {check_cfg.strategy}")
    ...
    def create_identity(
        self, with_datasource: bool | str = False, with_filename: bool = False, check_cfg: CheckCfg | None = None
    ) -> str:
        check_cfg: CheckCfg = check_cfg or self.check_cfg
        from soda.common.yaml_helper import to_yaml_str
        ...
        if isinstance(check_cfg.source_configurations, dict):
            ...
            # Group by check ignores query (inconsistent with other checks!) and checks configuration (also inconsistent with other checks!)
            if isinstance(check_cfg, GroupByCheckCfg):
                identity_source_configurations.pop("query", None)
                identity_source_configurations.pop("checks", None)

            # Exclude hyperparameters / tuning configurations from identity for anomaly detection checks
    ...
    def create_migrate_identities(self) -> dict[str, str] | None:
        ...
        with_datasource_yes = True
        with_datasource_no = False

        migrate_data_source_name = self.data_source_scan.data_source.migrate_data_source_name
        check_cfg: CheckCfg = self.check_cfg

        if migrate_data_source_name and self.data_source_scan.data_source.data_source_name != migrate_data_source_name:
            create_identities = True
            with_datasource_yes = migrate_data_source_name

        if isinstance(check_cfg.source_configurations, dict):
            # If group check identity exists, create migrate identities without it.
            group_check_identity = check_cfg.source_configurations.get("group_check_identity")
            if group_check_identity:
                create_identities = True
                check_cfg = copy.deepcopy(check_cfg)
                check_cfg.source_configurations.pop("group_check_identity", None)

        if create_identities:
            identities = {
                "v1": self.create_identity(
                    with_datasource=with_datasource_no,
                    with_filename=False,
                    check_cfg=check_cfg,
                ),
                "v2": self.create_identity(
                    with_datasource=with_datasource_yes,
                    with_filename=False,
                    check_cfg=check_cfg,
                ),
                "v3": self.create_identity(
                    with_datasource=with_datasource_yes,
                    with_filename=True,
                    check_cfg=check_cfg,
                ),
