...
class CloudScanMixin:
    ...
    def validate_prerequisites(self) -> bool:
        ...
        if self._configuration.soda_cloud.api_key_id and self._configuration.soda_cloud.api_key_secret:
            ...
            if login_response_code == 401:
                ...
                raise AssertionError(
                    "Your Soda Cloud API keys are invalid. Contact your Soda Admin or support@soda.io."
                )

        whoami_response_code, whoami_response = self._configuration.soda_cloud.who_am_i()

        if whoami_response_code == 200:
            self._configuration.soda_cloud.org_id = whoami_response["organisationId"]

            org_info = None
            for org in whoami_response["organisations"]:
                if org["id"] == whoami_response["organisationId"]:
                    org_info = org
                    break

            self._configuration.soda_cloud.organization_info = org_info
            self._configuration.soda_cloud.user = whoami_response["user"] if "user" in whoami_response else None
        else:
            raise AssertionError("Invalid Soda Cloud token. Contact your Soda Admin or support@soda.io.")
